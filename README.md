...
API para Python para crear archivos de tipo diccionario (llave+valor), con propiedades únicas e innovadoras que permiten encontrar una llave de manera instantánea en cualquier tipo de ordenador, a diferencia de [Redis](https://aws.amazon.com/es/redis/) DelosData proporciona una escritura portable, sencilla, eficaz y productiva, DelosData más que una alternativa es tu mejor opción para grandes volúmenes de datos.
...
**Rendimiento** Por ejemplo ahora Facebook, podría acceder a los datos de logeo de un usuario en una BD que tenga más de 300 millones de llaves, en solo 0.02 milli-segundos desde un simple ordenador domestico de gama baja, que tenga como mínimo: un procesador de un solo núcleo, 160 GB de disco y 256 MB de RAM.

**Compresión** La estructura de una BD de DelosData esta acondicionada para que pueda ser comprimida con `7z` con una tasa de compresión de 14% o más, haciendo que sea fácil el respaldo o la transferencia de un servidor a otro.

**SSD gen4** DelosData es ideal para esta nueva tecnología de lectura & almacenamiento ultra-rápido, ya que permite leer una llave en una BD que tenga 4 trillones de llaves en solo 0.03 `nano segundos`

---

> Python: ejemplo de uso para el NLP
...
Abriendo archivo e insertando una nueva llave, `¿como saber si una llave existe?`, cada vez que insertas una llave, el sistema detecta automáticamente si existe o no !para evitar eventos de corrupción!, DelosData es ideal en NLP para tareas como, `Byte Pair Encoding` & `Token embeddings`
...
## ¿Como instalar DelosData en Python?
...
*PASO 3*
- Ya puedes instalar DelosData, ejecutando el archivo [install.py](install.py)

*FIN*
- Felicidades ya puedes usar DelosData en Python :)
...
dict_db.add(llave, "no tengo dinero")
...
1. Usalo como si fuera un XML, JSON o TXT, lo que hará la diferencia sera la velocidad de consulta y seguridad de los datos guardados
...
**Soporte para Python3** soporte garantizado para Python3.
...
**Re-escritura** DelosData esta hecho en Python desde 0 con un rendimiento asombroso e envidiable, aun así la idea es re-codificar el núcleo de DelosData en C++, de esa manera el rendimiento y velocidad de consulta sera 7 veces más rápido en lectura y escritura, de esta manera el proyecto estará terminado al 100%.
